...
var RoomForm = React.createClass({
    ...
    joinRoomHandler: function() {
        this.channel.publish('Room.Join', {
            roomName: React.findDOMNode(this.refs.roomName).value
        });
    },
    render: function() {
...
var UserView = React.createClass({
    render: function() {
        var name = this.props.userName ? this.props.user.get('user') : null;
        return React.DOM.div(null,
            React.DOM.img({
                src: this.props.user.image(this.props.size)
        ...
        return React.DOM.ul({ className: 'list-unstyled'}, 
            this.props.collection.map(function(user) {
                if (me.id !== user.get('id'))
                    return React.DOM.li(null, React.createElement(UserView, {user: user, size: 50, userName: true}));
        }));
    }
});
...
var ChatMessage = React.createClass({
    render: function() {
        ...
        return React.DOM.li(null,
            ...
            React.DOM.div({className: pull}, React.createElement(UserView, {user: this.props.chat.get('user'), size: 20, userName: true})),
            React.DOM.div({className: 'clearfix'}, null),
            React.DOM.small({className: pull}, timeAgo),
            React.DOM.div({className: 'clearfix'}, null)
        );
    }
});
    ...
    render: function() {
        ...
        return React.DOM.ul({className: 'list-unstyled'},
            this.props.chats.map(function(chat) {
                return React.createElement(ChatMessage, {chat: chat, me: me});
        }));
    }
});
...
var ChatForm = React.createClass({
    ...
    formSubmit: function(e) {
        ...
        if (message !== '') {
            ...
            React.findDOMNode(this.refs.message).placeholder = '';
        } else 
            React.findDOMNode(this.refs.message).placeholder = 'Please enter a message';
    },
    render: function() {
        return React.DOM.div({className: 'row'},
            React.DOM.form({onSubmit: this.formSubmit}, 
                React.DOM.div({className: "col-sm-2"}, React.createElement(UserView, {user: this.props.me, size: 50, userName: true})),
                React.DOM.div({className: 'col-sm-8'}, React.DOM.input({type: 'text', className: 'form-control', ref: 'message'}, null)),
                React.DOM.div({className: 'col-sm-2'}, React.DOM.button({className: 'btn btn-primary'}, 'Send'))
        ));
    }
});
    ...
    render: function() {
        return React.DOM.div({className: 'row'},
            React.DOM.div({className: 'row'},
                React.DOM.div({className: 'col-sm-2'}, React.createElement(UserList, {collection: this.props.users, me: this.props.me})),
                React.DOM.div({className: 'col-sm-8 chat-list', ref: 'chatList'}, React.createElement(ChatList, {chats: this.props.chats, me: this.props.me}))
            ),
            React.createElement(ChatForm, {me: this.props.me})
        );
    }
});
